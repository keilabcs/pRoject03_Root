...
################################################# Inicio de Predicion NCM 
# Um algoritmo de predição de palavras e NCM.
#Nossos propósitos de tentar construir um preditor de próxima palavra e NCM.
...
y2 <- ggplot(tail(wordCountsN1_asc,n=50), 
             aes(x=wordsN1,y=Freq)) +
  geom_bar(stat="identity") + coord_flip() + 
...
#Plano de desenvolvimento para criar um algoritmo de previsão e uma aplicação shiny
...
w <- predict("73211100 fogao",wordCountsN3,10)
w

w <- predict("73211100",wordCountsN3,10)
w

w <- predict("fogao",wordCountsN3,10)
w
